# Creating an OAuth2 auth recipe

This tutorial shows how operators of a channel with OAuth2 can create a conda package
...
This is meant to simplify using channels with OAuth2 authentication/auhtorization, and
it avoids making users configure all the various options for OAuth2 themselves.

---

## The `channel_settings` format

conda-auth reads OAuth2 configuration from the `channel_settings` key in the
...
channel_settings:
  - channel: https://repo.example.com         # URL of your protected channel
    auth: oauth2                               # tells conda-auth to use OAuth2
...
build:
  number: 0
  script: |
    mkdir -p "${PREFIX}/etc/conda/condarc.d"
...
```bash
# Build
rattler build resource-handler-auth/
...
If the configuration is correct, the OAuth2 flow starts immediately with no
...
conda remove --name base resource-handler-auth
```
...
# 2. Log in — no OAuth flags required
conda auth login https://repo.example.com
```

conda-auth reads the `channel_settings` from the installed config and
initiates the OAuth2 flow automatically.  After a successful login, all
