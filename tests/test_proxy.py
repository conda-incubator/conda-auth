...
@pytest.mark.parametrize("raises", (False, True), ids=("stored", "storage-error"))
def test_get_credential_handles_storage_errors(monkeypatch, proxy_manager, raises):
    ...
    def get_credential(target):
        assert target == record.target
        if raises:
            raise CondaAuthError("Credential storage is unavailable")
        return record

    monkeypatch.setattr("conda_auth.storage.storage.get_credential", get_credential)

    assert proxy_manager.get_credential(record.target) == (None if raises else record)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("http://[::1]:8080", "http://user:password@[::1]:8080"),
    ),
    ids=("hostname", "ipv6"),
)
def test_proxy_url_with_credentials(proxy_url, expected):
    assert ProxyURL(proxy_url).with_credentials("user", "password") == expected


@pytest.mark.parametrize(
    ("proxy_value", "raise_if_missing", "message"),
    (
        (None, False, None),
        (123, False, None),
        (123, True, "Proxy URL must be text"),
    ),
    ids=("missing", "non-text-ignored", "non-text-error"),
)
def test_resolve_url_handles_missing_or_non_text_values(
    proxy_manager,
    proxy_value,
    raise_if_missing,
    message,
    ...
    if message is not None:
        with pytest.raises(CondaAuthError, match=message):
            proxy_manager.resolve_url(
                "http",
                proxy_servers={"http": proxy_value},
...
def test_status_entries_rejects_proxy_url_without_key(proxy_manager):
    with pytest.raises(CondaAuthError, match="Proxy URL status requires a proxy key"):
        proxy_manager.status_entries(proxy_url="http://proxy.example.com:8080")
...
@pytest.mark.parametrize(
    ...
    ids=("invalid-key", "invalid-url"),
)
def test_add_credentials_ignores_invalid_entries(proxy_manager, proxy_servers):
    def get_credential(target):
        raise AssertionError(f"Invalid proxy entry should not load credentials for {target!r}")

    assert (
        proxy_manager.add_credentials(proxy_servers, credential_getter=get_credential)
        == proxy_servers
    )
...
@pytest.mark.parametrize(
    ...
    (
        frozendict(),
        frozendict({"http": "http://proxy.example.com:8080"}),
    ),
    ids=("empty", "no-stored-credential"),
)
def test_apply_to_context_skips_unchanged_proxy_servers(
    monkeypatch,
    proxy_manager,
    proxy_servers,
):
    fake_context = FakeProxyContext(proxy_servers=proxy_servers)

    class FakeCondaSession:
        @staticmethod
        def cache_clear():
            raise AssertionError("Unchanged proxy settings should not clear the session cache")

    monkeypatch.setattr("conda_auth.proxy.context", fake_context)
    monkeypatch.setattr("conda_auth.proxy.CondaSession", FakeCondaSession)

    proxy_manager.apply_to_context(credential_getter=lambda target: None)

    assert fake_context._cache_ == {}
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("http://user:password@[::1]:8080", "http://[::1]:8080"),
        ("http://user:password@proxy.example.com:not-a-port", "http://proxy.example.com"),
    ),
    ids=("hostname", "ipv6", "invalid-port"),
)
def test_redact_proxy_url_removes_embedded_credentials(proxy_url, expected):
    assert ProxyURL(proxy_url).redacted() == expected
